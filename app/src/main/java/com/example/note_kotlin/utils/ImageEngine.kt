 ...
 */
class ImageEngine  //单例模式，私有构造方法
private constructor() : com.huantansheng.easyphotos.engine.ImageEngine {
    /**
     * 加载图片到ImageView
     *
     * @param context   上下文
    ...
    companion object {
        //单例
        var instance: ImageEngine? = null
            //获取单例
            get() {
                if (null == field) {
                    synchronized(ImageEngine::class.java) {
                        if (null == field) {
                            field = ImageEngine()
                        }
                    }
